...
export default function GroupCall({ roomCode: rawRoomCode }) {
  ...
  return (
    ...
    >
      ...
      {/* In-Call Chat Slide-Over Panel */}
      {chatOpen && (
        <div className="absolute inset-0 md:inset-y-0 md:right-0 md:left-auto md:w-88 md:max-w-[85vw] bg-[#0c0f14]/95 md:bg-[#0c0f14]/90 md:backdrop-blur-2xl flex flex-col md:border-l border-white/10 z-40 shadow-2xl animate-fade-in-up">
          <div className="px-4 py-3.5 border-b border-white/10 font-display font-bold text-sm text-white flex items-center justify-between">
            ...
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded-full text-white/50 hover:text-white hover:bg-white/10 active:scale-90 transition-all cursor-pointer"
              aria-label="Close chat"
            >
              <X className="w-4 h-4" strokeWidth={2} />
            </button>
          </div>
          <div className="flex-1 min-h-0 overflow-y-auto thin-scrollbar p-3.5 flex flex-col gap-2.5">
            {chatMessages.length === 0 && (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-4 text-white/40 gap-2">
                <MessageCircle className="w-8 h-8 text-white/20" />
                <p className="text-xs">No messages yet. Send a message to participants in this room.</p>
              </div>
            )}
            {chatMessages.map((m, i) => (
              <div key={i} className="flex flex-col gap-0.5 bg-white/[0.05] border border-white/5 rounded-xl p-2.5">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-xs text-gold flex items-center gap-1">
                    <span
                      className="w-2 h-2 rounded-full inline-block"
                      style={{ backgroundColor: colorForName(m.username) }}
                    />
                    {m.username}
                  </span>
          ...
          <form onSubmit={sendChatMessage} className="p-3 border-t border-white/10 flex gap-2 bg-black/30">
            <input
              type="text"
              value={chatInput}
              onChange={(e) => setChatInput(e.target.value)}
              placeholder="Send message to room…"
              className="flex-1 min-w-0 bg-white/10 text-white placeholder:text-white/40 rounded-xl px-3.5 py-2 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-brand"
            />
            <button
              type="submit"
              disabled={!chatInput.trim()}
              aria-label="Send"
              className="w-9 h-9 shrink-0 rounded-xl bg-brand-gradient hover:brightness-110 active:scale-95 disabled:opacity-40 transition-all flex items-center justify-center text-white cursor-pointer shadow-sm"
            >
